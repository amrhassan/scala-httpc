...
private [httpc] trait Convenient {
  ...
  def get[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
    request(Method.Get, url, data)

  def put[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
    request(Method.Put, url, data)

  def patch[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
    request(Method.Patch, url, data)

  def head[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
    request(Method.Head, url, data)

  def post[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
    request(Method.Post, url, data)

  def delete[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
    request(Method.Delete, url, data)

  def trace[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
    request(Method.Trace, url, data)

  def options[A: ToRequest](url: String, data: A = Array.empty[Byte]): Httpc[Response] =
